#!/usr/bin/env python
"""
md-SiO-gibbs-liquid.py VERSION 1.0
Author: Brian Boates

compute Gibbs for SiO liquid
"""
import os, sys, commands, glob, time

global massSi, massO
massSi = 28.0855  # in amu
massO  = 15.9994

global tFinal
tFinal = 8  # in picoseconds
...
def main():
    ...
    nSi = int(commands.getoutput("head -6 POSCAR | tail -1").split()[0])
    ...
    out.write(temp+'\n')
    out.write(str(massSi)+'\n')
    out.close()
    os.system("fluidicity.x < fluidicity_Si.in > fluidicity_Si.out")
    ...
    out.write(temp+'\n')
    out.write(str(massSi)+'\n')
    fluidicity = commands.getoutput('tail -n-1 fluidicity_Si.out').split()[-1]
    out.write(fluidicity+'\n')
    out.close()
    os.system('entropy_liquid.x < entropy_Si.in > entropy_Si.out')

    os.system("grep 'Entropy per atom in eV/K' entropy_Si.out | awk '{print $6}' > S_Si") # in eV/K
    os.system("grep 'Entropy per atom in k_B'  entropy_Si.out | grep -v id | awk '{print $6}' > S_Si_in_kB") # in kB
    ...
    ######
    os.system("paste S_Si S_O | awk '{print $1+$2}' > S")
    os.system("paste S_Si_in_kB S_O_in_kB | awk '{print $1+$2}' > S_in_kB")
    ######
