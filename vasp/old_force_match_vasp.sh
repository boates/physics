...
set acell=`grep -A1 'length of vectors' OUTCAR | head -2 | tail -1 | awk '{print $1/0.5291772}'`
echo "acell: " $acell
...
sed s/acell/"$acell"/g /home/boates/bin/TEMPLATE.in | sed s/number_of_snapshots/"$number_of_snapshots"/g | sed s/half_box/"$half_box"/g | sed s/natoms/"$natoms"/g > Input.in
