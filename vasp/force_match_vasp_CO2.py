#!/usr/bin/env python
"""
force_match_vasp_CO2.py
Author: Brian Boates

Perform force-matching analysis on VASP output
to obtain pair-potentials for CO2.
...
#    os.system("grep -A"+str(natom+1)+" 'TOTAL-FORCE' "+f+" | grep -v TOTAL | grep -v '\-\-' > position_force")
    ...
    os.system('sed s/"ACELL"/"'+str(a)+'"/g /home/boates/bin/TEMPLATE_CO2.in | sed s/"BCELL"/"'+str(b)+'"/g | sed s/"CCELL"/"'+str(c)+'"/g > tmp.in')
    os.system('sed s/"NSTEPS"/"'+str(Nsteps)+'"/g tmp.in | sed s/"HALFBOX"/"'+str(half)+'"/g > Input.in')
