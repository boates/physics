#!/usr/bin/env python
"""
md-MgSiO3-gibbs-liquid.py VERSION 1.0
Author: Brian Boates

compute Gibbs for MgSiO3 liquid from vasp 5
"""
...
#    os.system('rm -f VDOS_O.in VDOS_O.out')
    ...
    out.write(temp+'\n')
    out.write(str(massMg)+'\n')
    out.close()
    os.system("fluidicity.x < fluidicity_Mg.in > fluidicity_Mg.out")
    ...
    out.write(temp+'\n')
    out.write(str(massMg)+'\n')
    fluidicity = commands.getoutput('tail -n-1 fluidicity_Mg.out').split()[-1]
    out.write(fluidicity+'\n')
    out.close()
    os.system('entropy_liquid.x < entropy_Mg.in > entropy_Mg.out')
