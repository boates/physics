#!/usr/bin/env python
"""
md-monatomic-liquid_analyze.py VERSION 1.0
Author: Brian Boates
...
global mass
mass = 14.00674  # N in amu
#mass = 15.9994  # O in amu
#mass = 28.0855  # Si in amu
...
def main():
    ...
    out.write('POSCAR\n')
    out.close()
    os.system('nice -15 cbn_from_xyz_general.x < cbn.in > cbn.out')
...
#    os.system("awk '{print $1*"+str(step_in_fs/1000.0)+",$2}' correlator_16nn.dat > c16.dat")
    ...
    os.system('mv -f *.dat *.xyz *.cnn *.mol *.in *.blocker *.hist *.avg *.cbn date analysis/')
    os.system('mv -f *.out toten E P V T S S_in_kB PV TS H F G analysis/')
