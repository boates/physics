!       program forces_molecular_centers.f90
!***********************************************************
!       Create an fxyz file of molecule-COMs
...
!***********************************************************
        ...
        real*8 Fx(maxAtoms), Fy(maxAtoms), Fz(maxAtoms), Fxcom, Fycom, Fzcom
        ...
        open(3,file='CENTERS.xyzf')

 5      format(f12.6,x,f12.6,x,f12.6,x,f12.6,xf12.6,x,f12.6)
        ...
        do i=1,maxSteps
          ...
          do j=1,natom
            read(1,*) typat, X(j), Y(j), Z(j), nn(j)
            read(2,*) Fx(j), Fy(j), Fz(j)
            nn(j) = nn(j) + 1
          enddo

          k = 0
          write(3,*) "POSITION                                       TOTAL-FORCE (eV/Angst)"
          do j=1,natom
            ...
            if (tracker(i,nn(j)).ne.-1) then
              ...
              write(3,5) Xcom*0.529177, Ycom*0.529177, Zcom*0.529177, Fxcom, Fycom, Fzcom
