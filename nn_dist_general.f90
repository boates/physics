...
integer natom, natom1, natom2, natom_cnn, tstep, nsteps, nn(maxAtoms)
...
! Total number of atoms that will be in cnn file
if (typat1.ne.typat2) natom_cnn = natom1 + natom2
if (typat1.eq.typat2) natom_cnn = natom1
...
do i = 1, maxSteps
  ...
  ! Write type 2 atoms to cnn file
  do k = 1, natom
    if (typat(k).eq.typat2) then
      write(2,5) typat(k), X(k), Y(k), Z(k), (0,l=1,nwrite)
    endif
  enddo
