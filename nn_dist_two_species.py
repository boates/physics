...
def main():
    ...
    try:
        ...
        # nO is the number of atoms of the 2nd species, distribution calculated w.r.t 1st species
        print '\n usage: '+sys.argv[0]+' number_of_bins alat_x alat_y alat_z(bohr) FILE.xyz nO label(optional)\n'
        sys.exit(0)
    ...
    cnnFile.write('# number_of_particles = '  + str(number_of_particles - nO) + '\n')
    cnnFile.write('# number_of_neighbours = ' + str(nO) + '\n')
    cnnFile.write('#\n')
    cnnFile.write('#\n')
    cnnFile.write('#\n')
    ...
    histogram = numpy.zeros((int(nO), int(number_of_bins)), dtype=numpy.int)
    ...
    for s in range(number_of_snapshots):
        ...
        for p in range(number_of_particles - nO):
             
            cnnFile.write(typat[p] + ' % .8e' % SNAPSHOT_array[p][0] + ' % .8e' % SNAPSHOT_array[p][1] + ' % .8e' % SNAPSHOT_array[p][2])
    
            neighbour_distances = [] 
    
            for o in range(number_of_particles - nO,number_of_particles):
            ...
            for i in range(nO):
                distance_array[-1].append(neighbour_distances[i][0])
                cnnFile.write(' ' + str.rjust(str(neighbour_distances[i][1]), 3) ) 
            cnnFile.write('\n')
    
        # At this point, distance_array is ready to go
    
        for distance_list in distance_array:
            for neighbour_index in range(nO):
