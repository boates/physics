...
def main():
    ...
    tracker = {}
    O2_tracker = []
    cbn_O2 = open('O2.cbn','w')
    cbn_O2.write(' # comment = \n')
    cbn_O2.write(' # a = '+str(alat)+'\n')
    cbn_O2.write(' # b = '+str(alat)+'\n')
    cbn_O2.write(' # c = '+str(alat)+'\n')
    cbn_O2.write(' # natom = '+str(256)+'\n')
    cbn_O2.write(' # nneighbors = 1 \n')
    cbn_O2.write(' # \n')
    cbn_O2.write(' # \n')
    cbn_O2.write(' # \n')
    cbn_O2.write(' # units = angstrom \n')

    out_OO = open('OO.bond','w')
    count = 1000
    ...
    for i in range(nsteps):
        ...
        for molecule in molecules:
            ...
            molecules.sort()
            if species == 'C0O2':
                O2_tracker[i].append( molecule )
                out_OO.write(str(molecule[0])+' '+str(molecule[1])+'\n')
                

        # Keep a count of total number of molecules found for normalization
        nmolecules += len(molecules)

        # Write a cbn file for O2
        for j in range(natom):
            if typat[j] == 'O':
                cbn_O2.write(typat[j]+' '+str(x[j])+' '+str(y[j])+' '+str(z[j])+' ')
                dimer = False
                for p in O2_tracker[i]:
                    if j in p:
                        dimer = [k for k in p if k != j][0]
                if dimer:
                    cbn_O2.write(str(dimer+1)+'\n')
                else:
                    cbn_O2.write(str(count)+'\n')
                    count += 1
