...
def main():
    ...
    for rs in RS:
        ...
        try:
            P = commands.getoutput("tail -2 "+rs+"/analysis/pressure.blocker | head -1 | awk '{print $4}'")
        except:
            P = '----'

        try:
            os.system("tail -"+NLINES1+" "+rs+"/analysis/D.dat | awk '{print $2}' > D.tmp")
            d1 = commands.getoutput("blocker D.tmp | tail -2 | head -1 | awk '{print $4}'")
            os.system("tail -"+NLINES2+" "+rs+"/analysis/DiffCoef.dat | awk '{print $2}' > DiffCoef.tmp")
